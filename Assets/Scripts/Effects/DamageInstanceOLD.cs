...
[Serializable]
public class DamageInstanceOLD : Effect
{
    [SerializeReference]
    public List<IStatTag> stats;
    [HideInInspector]
    ...
    public override void Activate()
    {
        ...
        if (!preventTriggers)
        {
            Entity triggerOwner = triggerPlayerOwner ? Owner.GetMechanic<Mechanic_PlayerOwner>().Owner : Owner;
            ...
            if(Owner.GetMechanic<Mechanic_Magic>().enchantedAttacks.Count > 0)
                runes.AddRange(Owner.GetMechanic<Mechanic_Magic>().enchantedAttacks.Dequeue());
    ...
    public void AddModifiers(DamageSolver calculation)
    {
        ...
        foreach (DamageSolver modifier in damageModifiers)
        {
    ...
    {
        DamageInstanceOLD clone = (DamageInstanceOLD)base.Clone();

...
public class Stat_PlayerOwner : Mechanic<Stat_PlayerOwner>
{
    ...
    public void ApplyContribution(Entity target, float effectMultiplier)
    {
        ...
        target = target.GetMechanic<Stat_PlayerOwner>().playerEntity;
        Viewer player = playerEntity.GetMechanic<Stat_PlayerOwner>().player;
        if (!target.GetMechanic<Stat_PlayerOwner>().assists.TryAdd(player, Mathf.Abs(effectMultiplier)))
        {
            target.GetMechanic<Stat_PlayerOwner>().assists[player] += Mathf.Abs(effectMultiplier);
        }
